...
def rmt_frechet_distance_analytical(
    mu1: NDArray, sigma1: NDArray, mu2: NDArray, sigma2: NDArray, n: int, eps : float = 1e-12,
) -> float:
    """
    ...
    # ------------------------------
    lambda_ = np.sort(np.real(np.linalg.eigvals((sigma1 + offset) @ (sigma2 + offset))))
    ...
    # TODO: check me!!!
    eta = np.clip(eta, 0, None) # <- is this extremely weird?
...
def rmt_fid_from_feats(feats1: NDArray, feats2: NDArray, eps: float = 1e-12) -> float:
    assert len(feats1) == len(feats2)
    mu1, sig1 = compute_statistics(feats1)
    mu2, sig2 = compute_statistics(feats2)
    return rmt_frechet_distance_analytical(mu1, sig1, mu2, sig2, len(feats1), eps=eps)
